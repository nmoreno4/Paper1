...
# -*- coding: utf-8 -*-
"""
Created on Thu May 11 14:15:41 2017
...
# Create the dictionnary to insert
#stockcollection = []
#for stock in set(FF_factors['TICKER']):
#    if type(stock) != float:
#        stockdocument = {}
#        #stockdocument['ticker'] = stock.decode('UTF-8')
#        for y in range(2005,2006):
#            stockdocument['year'] = datetime.datetime(y,1,1)
#            stockdocument['values'] = {}
#            for m in range(1,13):
#                stockdocument['values'][str(m)] = {}
#                for d in range(1,32):
#                    stockdocument['values'][str(m)][str(d)] = {}
#        stockcollection.append(stockdocument)
...
#collection.insert_many(stockcollection)
#collection.create_index('year')


#%%
for ix, entry in FF_factors.iterrows():
    PERMNO = entry['PERMNO']
    if type(entry['TICKER']) != float:
        TICKER = entry['TICKER'].decode('UTF-8')
    if type(entry['CUSIP']) != float:
        CUSIP = entry['CUSIP'].decode('UTF-8')
    if type(entry['COMNAM']) != float:
        COMNAM = entry['COMNAM'].decode('UTF-8').lower()
    if type(entry['Size']) != float:
        Size = entry['Size'].decode('UTF-8')
    if type(entry['BM']) != float:
        BM = entry['BM'].decode('UTF-8')
    if type(entry['ROE_']) != float:
        ROE_ = entry['ROE_'].decode('UTF-8')
    if type(entry['Inv_']) != float:
        Inv_ = entry['Inv_'].decode('UTF-8')
    weight_port_1Month = float(entry['weight_port_1Month'])
    weight_port_6Month = float(entry['weight_port_6Month'])
    weight_port_3Month = float(entry['weight_port_3Month'])
    weight_port = float(entry['weight_port'])
    retadj = float(entry['retadj'])
    DATE = epoch + datetime.timedelta(days=int(entry['DATE']))
    if type(entry['Portfolio_2']) != float:
        Portfolio_2 = entry['Portfolio_2'].decode('UTF-8')
    if type(entry['Portfolio_3']) != float:
        Portfolio_3 = entry['Portfolio_3'].decode('UTF-8')
    if type(entry['Portfolio_4']) != float:
        Portfolio_4 = entry['Portfolio_4'].decode('UTF-8')
    if DATE.year == y_start:
        collection.insert_one(
                              {
                                 ...
                                 "weight_port_1Month" : weight_port_1Month,
                                 "weight_port_3Month" : weight_port_3Month,
                                 "weight_port_6Month" : weight_port_6Month,
                                 "INV" : Inv_,
                                 "ROE" : ROE_,
                                 "BM" : BM,
                                 "SIZE" : Size,
...
#        CUSIP = entry['CUSIP'].decode('UTF-8')
#    if type(entry['COMNAM']) != float:
#        COMNAM = entry['COMNAM'].decode('UTF-8')
#    if type(entry['Size']) != float:
#        Size = entry['Size'].decode('UTF-8')
#    if type(entry['BM']) != float:
#        BM = entry['BM'].decode('UTF-8')
...
#    if type(entry['Inv_']) != float:
#        Inv_ = entry['Inv_'].decode('UTF-8')
#    weight_port_1Month = entry['weight_port_1Month']
#    weight_port_6Month = entry['weight_port_6Month']
#    weight_port_3Month = entry['weight_port_3Month']
#    weight_port = entry['weight_port']
#    retadj = entry['retadj']
#    DATE = epoch + datetime.timedelta(days=int(entry['DATE']))
#    if type(entry['Portfolio_2']) != float:
#        Portfolio_2 = entry['Portfolio_2'].decode('UTF-8')
...
#            return False
#    
#    for crt_y in range(2005,2006):
#        for crt_m in range(1,13):
#            for crt_d in range(1,31):
#                checkKeyValuePairExistence(stockcollection, 'TICKER', TICKER)
#                
#                try:
#                    stockcollection['ticker'==TICKER][crt_y == DATE.year][crt_m == DATE.month][crt_d == DATE.day]['permno'] = PERMNO
#                except KeyError:
#                    continue
#                try:
#                    stockcollection['TICKER'==TICKER][crt_y == DATE.year][crt_m == DATE.month][crt_d == DATE.day]['ticker'] = TICKER
#                except KeyError:
#                    continue
#                try:
#                    stockcollection['TICKER'==TICKER][crt_y == DATE.year][crt_m == DATE.month][crt_d == DATE.day]['cusip'] = CUSIP
#                except KeyError:
#                    continue
#                try:
#                    stockcollection['TICKER'==TICKER][crt_y == DATE.year][crt_m == DATE.month][crt_d == DATE.day]['size'] = Size
#                except KeyError:
#                    continue
#                try:
#                    stockcollection['TICKER'==TICKER][crt_y == DATE.year][crt_m == DATE.month][crt_d == DATE.day]['bm'] = BM
#                except KeyError:
#                    continue
#                try:
#                    stockcollection['TICKER'==TICKER][crt_y == DATE.year][crt_m == DATE.month][crt_d == DATE.day]['roe'] = ROE_
#                except KeyError:
#                    continue
#                try:
#                    stockcollection['TICKER'==TICKER][crt_y == DATE.year][crt_m == DATE.month][crt_d == DATE.day]['inv'] = Inv_
#                except KeyError:
#                    continue
#                try:
#                    stockcollection['TICKER'==TICKER][crt_y == DATE.year][crt_m == DATE.month][crt_d == DATE.day]['retadj'] = retadj
#                except KeyError:
#                    continue
#
#                 
...
#        row = 0
##        X = np.array([['altId', 'firstCreated', 'guId', 'assetId', 'firstMentionSentence', 'assetCodes', 'assetName', 'linkedIds', 'noveltyCounts', 'relevance', 'sentimentClass', 'sentimentNegative', 'sentimentNeutral', 'sentimentPositive', 'sentimentWordCount', 'volumeCounts', 'bodySize', 'companyCount', 'exchangeAction', 'headlineTag', 'marketCommentary', 'sentenceCount', 'wordCount', 'headline', 'language', 'urgency', 'subjects', 'provider', 'sourceTimestamp', 'audiences', 'feedTimestamp', 'takeSequence']])
#        X1 = [['altId', 'firstCreated', 'guId', 'assetId', 'firstMentionSentence', 'assetCodes', 'assetName', 'linkedIds', 'noveltyCounts', 'relevance', 'sentimentClass', 'sentimentNegative', 'sentimentNeutral', 'sentimentPositive', 'sentimentWordCount', 'volumeCounts', 'bodySize', 'companyCount', 'exchangeAction', 'headlineTag', 'marketCommentary', 'sentenceCount', 'wordCount', 'headline', 'language', 'urgency', 'subjects', 'provider', 'sourceTimestamp', 'audiences', 'feedTimestamp', 'takeSequence']]
#        df = pd.DataFrame(columns = ['altId', 'firstCreated', 'guId', 'assetId', 'firstMentionSentence', 'assetCodes', 'assetName', 'linkedIds', 'noveltyCounts', 'relevance', 'sentimentClass', 'sentimentNegative', 'sentimentNeutral', 'sentimentPositive', 'sentimentWordCount', 'volumeCounts', 'bodySize', 'companyCount', 'exchangeAction', 'headlineTag', 'marketCommentary', 'sentenceCount', 'wordCount', 'headline', 'language', 'urgency', 'subjects', 'provider', 'sourceTimestamp', 'audiences', 'feedTimestamp', 'takeSequence'])
#
#        #%%
#        end = time.time()
#        start = time.time()
...
#            assetCodes =  item["analytics"]["analyticsScores"][0]["assetCodes"]
#            assetName =  item["analytics"]["analyticsScores"][0]["assetName"]
...
#                                        'analytics':[
#                                                    {'assetId':crtDf['assetId'].values[0], 'firstMentionSentence':crtDf['firstMentionSentence'].values[0].item(), 'assetCodes':crtDf['assetCodes'].values[0], 'assetName':crtDf['assetName'].values[0], 'linkedIds':crtDf['linkedIds'].values[0], 'noveltyCounts':crtDf['noveltyCounts'].values[0], 'relevance':crtDf['relevance'].values[0].item(), 'sentimentClass':crtDf['sentimentClass'].values[0].item(), 'sentimentNegative':crtDf['sentimentNegative'].values[0].item(), 'sentimentNeutral':crtDf['sentimentNeutral'].values[0].item(), 'sentimentPositive':crtDf['sentimentPositive'].values[0].item(), 'sentimentWordCount':crtDf['sentimentWordCount'].values[0].item(), 'volumeCounts':crtDf['volumeCounts'].values[0]}
#                                                    ],
